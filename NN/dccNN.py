...
class dccNN(nn.Module):
    def __init__(self,int):
        ...
        self.int=int
        self.layer1 = nn.Linear(in_features=3, out_features=int*64)  # Input layer adapted for 3 input features
        self.layer2 = nn.Linear(in_features=int*64, out_features=int*64)
        self.layer3 = nn.Linear(in_features=int*64, out_features=int*64)
        self.layer4 = nn.Linear(in_features=int*64, out_features=int*64)
        self.layer5 = nn.Linear(in_features=int*64, out_features=int*64)
        self.layer6 = nn.Linear(in_features=int*64, out_features=int*64)
        self.output_layer = nn.Linear(in_features=5*64, out_features=1)  # Output layer adapted for 1 output
        self.swish = Swish()
        self.elu=nn.ELU()
    
    def forward(self, x):
        x = self.elu(self.layer1(x))
        x = self.elu(self.layer2(x))+x
        x = self.elu(self.layer3(x))+x
        x = self.elu(self.layer4(x))+x
        x = self.elu(self.layer5(x))+x
        x = self.swish(self.layer6(x))+x
...
def get_new_data():
    ...
    v_values = (2*torch.pi)*(2*torch.rand(5*10**6,device=device,dtype=type)-1)    
    theta_values = 2*torch.pi*torch.rand(5*10**6,device=device,dtype=type)  # 45 degrees in radians
    r_values = 10*torch.rand(5*10**6,device=device,dtype=type) # r should be positive
...
print("test 1")
model = dccNN(5).to(dtype=type).to(device)
optimizer = optim.AdamW(model.parameters(), lr=1e-4,weight_decay=0.001)
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.999985)
# Training loop
epochs =30
...
print("test 2")
model = dccNN(5).to(dtype=type).to(device)
optimizer = optim.AdamW(model.parameters(), lr=1e-4,weight_decay=0.002)
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.999985)
...
print("test 3")
model = dccNN(5).to(dtype=type).to(device)
optimizer = optim.AdamW(model.parameters(), lr=1e-4,weight_decay=0.0005)
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.999985)
...
print("test5")
model = dccNN(5).to(dtype=type).to(device)
optimizer = optim.Adagrad(model.parameters(), lr=1e-4,weight_decay=0.001)
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.999985)
...
#torch.jit.save(traced_model, "dccNN2.pth")
