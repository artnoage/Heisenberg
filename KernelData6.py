...
for i in range(1000):
    values[i*points_per_iteration:(i+1)*points_per_iteration,:]=logkernelcal(point[i*points_per_iteration:(i+1)*points_per_iteration,:])
    print("you are in the", i, "collection")
    torch.save(torch.cat([point,values],dim=1),"data6.pt")